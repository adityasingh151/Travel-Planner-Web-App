...
export default function Header() {
  const { username, isLoggedIn, logout } = useAuth(); // Use the Auth context
  ...
  return (
    <header className="w-full h-20 flex items-center fixed z-50 justify-between px-8 bg-white shadow-md">
      ...
      <nav className="flex space-x-8 text-lg font-medium text-pink-600">
        ...
        <Link href="/services" className="relative group">Services</Link>
        <Link href="/packagesarchive" className="relative group">Upcoming Packages</Link>
      </nav>
